...
log_header "Initializing ${DOTFILES_PROFILE:-}/${DOTFILES_PLATFORM:-} (ops)..."

# Install Homebrew if not present (ops user needs its own Homebrew)
if ! command -v brew >/dev/null 2>&1; then
  log_info "Installing Homebrew..."
  /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
  log_warn "Follow Homebrew's post-install instructions to add it to your PATH, then re-run 'make ops-init'"
  exit 0
else
