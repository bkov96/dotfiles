...
log_header "Initializing ${DOTFILES_PROFILE:-}/${DOTFILES_PLATFORM:-} (admin)..."

# Install Xcode Command Line Tools if not present
if ! xcode-select -p >/dev/null 2>&1; then
  log_info "Installing Xcode Command Line Tools..."
  xcode-select --install
  log_warn "Complete the Xcode CLT installation dialog, then re-run 'make admin-init'"
  exit 0
else
...
if ! command -v brew >/dev/null 2>&1; then
  ...
  log_warn "Follow Homebrew's post-install instructions to add it to your PATH, then re-run 'make admin-init'"
  exit 0
else
...
if [ -f "$PROFILE_DIR/.config.example.json" ] && [ ! -f "$PROFILE_DIR/.config.json" ]; then
  ...
  log_warn "Run 'dotfiles configs unlock' before continuing to resolve secrets from Bitwarden"
else
